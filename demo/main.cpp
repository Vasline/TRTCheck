...
bool ReadLines(const std::string &path, std::vector<std::string> &content)
{
	std::ifstream fin(path, std::ios::binary);
	if (!fin.is_open())
	{
		std::string msg = "Open file faild! filename: " + path;
...
std::vector<std::vector<float>> getTrtResults(const std::string &onnx_path, const std::string &trt_save_path, bool fp16_flag)
{
	...
	// const std::string warn_layer_path = "./warn_layers_shapes.txt";
	// std::vector<std::string> set_output_vec;
	// if (!ReadLines(warn_layer_path, set_output_vec)) {
	// 	return output_data;
	// }
	// for (int i = 0;i < set_output_vec.size();++i) {
	// 	if (i != set_output_vec.size() - 1) {
	// 		set_output_vec[i].pop_back();
	// 	}
	// }

	// const std::string onnx_path = "./data/engine/audio_hbt_large_02s.onnx";
	std::vector<std::string> set_output_vec;
	...
	// std::uniform_real_distribution<> dis(0.0f, 1.0f); // 分布在 [0.0, 1.0] 之间的浮点数
	...
	bool status = trt_model->forward(input_data);
	std::vector<std::string> outputs_name = trt_model->getOutputsName();
	status = trt_model->getOutputResults(outputs_name, output_data);
	if (!status) {
...
int main(int argc, char **argv)
{
	...
	const std::string out_trt_fp32_path = argv[2];
	const std::string out_trt_fp16_path = argv[3];
	std::map<std::string, std::vector<float>> fp32_result = getTrtResults(onnx_path, out_trt_fp32_path, false);
	std::map<std::string, std::vector<float>> fp16_result = getTrtResults(onnx_path, out_trt_fp16_path, true);
